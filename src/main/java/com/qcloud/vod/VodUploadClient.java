...
public class VodUploadClient {
    ...
    public VodUploadResponse upload(String region, VodUploadRequest request) throws Exception {
        ...
        VodClient vodClient = new VodClient(credential, region);

        ApplyUploadResponse applyUploadResponse = applyUpload(vodClient, request);
        ...
        if (StringUtil.isNotEmpty(request.getMediaType()) && StringUtil.isNotEmpty(applyUploadResponse.getMediaStoragePath())) {
            uploadCos(transferManager, request.getMediaFilePath(), applyUploadResponse.getStorageBucket(), applyUploadResponse.getMediaStoragePath());
        }
     ...
     * COS上传
     */
    private void uploadCos(TransferManager transferManager, String localPath, String bucket, String cosPath) throws Exception {
